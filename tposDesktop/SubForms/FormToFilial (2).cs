...
namespace tposDesktop.SubForms
{
    public partial class FormToFilial : DesignedForm
    {
        ...
        public FormToFilial()
        {
            ...
            dRows = new Dictionary<int, DataSetTpos.productRow>();
        ...
        private void dgvTofilial_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            if (tscanner != null && (e.KeyChar == 13 && beginBarcode))
            {
                ...
                if (prows.Length > 0)
                {
                    ...
                    dRows.Add(pdrows[0].productId, pdrows[0]);
                    dgvTofilial.Refresh();
        ...
        Dictionary<int, DataSetTpos.productRow> dRows;
            ...
            if (svf.ShowDialog() == System.Windows.Forms.DialogResult.OK)
            {
                if (MessageBox.Show("Вы действительно хотите экспортировать список?", "Предупреждение...", MessageBoxButtons.YesNo, MessageBoxIcon.Asterisk) == System.Windows.Forms.DialogResult.Yes)
                {
                    ...
                    foreach (DataRow dr in table.Rows)
                    {
                        ...
                        DataSetTpos.productRow[] productRow = { dRows[id] };
                        ...
                        {
                            db.AddProduct(productRow, false, "", (Convert.ToInt32(dr["pack"]) != 0 ? Convert.ToInt32(dr["pack"]) * int.Parse(dr["Kol"].ToString()) : int.Parse(dr["Kol"].ToString())));
                            db.Debt();
                            int j = i - beginIndex;
                            ...
                            objVal[j, 3] = productRow[0].price.ToString();
                            objVal[j, 4] = (productRow[0].price * int.Parse(dr["Kol"].ToString())).ToString();
